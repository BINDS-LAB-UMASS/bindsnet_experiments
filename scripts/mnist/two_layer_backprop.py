...
if train:
    ...
    input_connection = Connection(source=input_layer, target=hidden_layer, weight_decay=0.01 / time)
    hidden_connection = Connection(source=hidden_layer, target=output_layer, weight_decay=0.001 / time)
    ...
    for c in network.connections:
        network.connections[c].update_rule.weight_decay = 0.0
...
for i, (image, label) in enumerate(zip(images, labels)):
    ...
    network.run(inpts={'X': image.repeat(time, 1)}, time=time)
    ...
    if train:
        ...
        network.connections['Y', 'Z'].w -= 0.01 * lr * grads['dl/dw2']
