"""
Diehl & Cook 2015 spiking neural network (SNN) architecture with noisy input. MNIST digits are binarized (0, 1 values)
and are randomly flipped with probability `p_flip` before being encoded as Poisson spike trains, which are used
as input to the SNN.
"""
...
parser.add_argument('--intensity', type=float, default=255 / 4)
...
for path in [params_path, curves_path, results_path]:
...
# Build network.
if train:
    network = DiehlAndCook2015(n_inpt=784, n_neurons=n_neurons, exc=excite, inh=inhib, dt=dt, norm=78.4,
                               theta_plus=0.05)
else:
...
for i in range(n_examples):
    ...
    if plot:
        ...
        if i == 0:
            ...
            weights_im = plot_weights(square_weights, wmax=1.0)
