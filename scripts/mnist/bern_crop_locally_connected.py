...
parser.add_argument('--max_prob', type=float, default=1.0)
...
model = 'bern_crop_locally_connected'
data = 'mnist'
...
if train:
    network = LocallyConnectedNetwork(
        ...
        n_filters=n_filters, inh=inhib, dt=dt, nu_pre=5e-3, nu_post=1e-2, theta_plus=theta_plus,
        theta_decay=theta_decay, wmin=0.0, wmax=1.0, norm=norm
    )
    network.connections[('X', 'Y')].update_rule = Hebbian(
...
for i in range(n_examples):
    ...
    sample = bernoulli(datum=image, time=time, max_prob=max_prob)
    inpts = {'X': sample}
...
if not os.path.isfile(os.path.join(results_path, name)):
    with open(os.path.join(path, name), 'w') as f:
        if train:
            f.write(
                'random_seed,kernel_size,stride,n_filters,crop,max_prob,n_train,inhib,time,timestep,theta_plus,'
            ...
            f.write(
                'random_seed,kernel_size,stride,n_filters,crop,max_prob,n_train,n_test,inhib,time,timestep,theta_plus,'
                'theta_decay,intensity,norm,progress_interval,update_interval,mean_all_activity,'
                'mean_proportion_weighting,mean_ngram,max_all_activity,max_proportion_weighting,max_ngram\n'
            )
