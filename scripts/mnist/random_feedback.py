...
if train:
    network = DiehlAndCook2015v2(
        n_inpt=784, n_neurons=n_neurons, exc=excite, inh=inhib, dt=dt, norm=78.4, theta_plus=theta_plus
    )
    network.connections['Y', 'Y'] = Connection(
        source=network.layers['Y'], target=network.layers['Y'], w=-10 + torch.randn(n_neurons, n_neurons)
    )
